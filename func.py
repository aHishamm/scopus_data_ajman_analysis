...
def process_scopus_df(filepath): 
    ...
    return df
def main_func(startyear,endyear,df): 
        new_df = df.copy() 
        ...
        fig1 = px.line(grouping,x='Year',y='Cited by', title='# of Citations per Year', markers = True)
        fig1.update_layout(
            xaxis = dict(
                tickmode = 'linear',
                dtick = 1
        ...
        fig2 = px.pie(reset_pivoted,values='count',names='Document Type',title='Breakdown by Document type: '+str(startyear)+' - '+str(endyear)) 
        new_df = df.copy() 
        min_year = new_df['Year'].min()
        max_year = new_df['Year'].max() 
        if startyear < min_year: 
        ...
        fig3 = px.pie(reset_pivoted,values='count',names='AU_ranking',title='Breakdown by AU Ranking: '+str(startyear)+' - '+str(endyear)) 
        new_df = df.copy() 
        min_year = new_df['Year'].min()
        max_year = new_df['Year'].max() 
        if startyear < min_year: 
        ...
        if startyear is not None and endyear is not None: 
            ...
            query = new_df[(new_df.Year >= startyear) & (new_df.Year <= max_year)]
        #pivoting 
        fig4 = px.pie(query,values='Cited by',names='Document Type',title='Citations by Document Type: '+str(startyear)+' - '+str(endyear)) 
        new_df = df.copy() 
        min_year = new_df['Year'].min()
        max_year = new_df['Year'].max() 
        if startyear < min_year: 
        ...
        fig5 = px.bar(reset_pivoted,y='count',x='Document Type',title='Breakdown by Document type: '+str(startyear)+' - '+str(endyear)) 
        new_df = df.copy() 
        min_year = new_df['Year'].min()
        max_year = new_df['Year'].max() 
        if startyear < min_year: 
        ...
        reset_pivoted = reset_pivoted.rename(columns={'AU_ranking':'AU_ranking','0':'count'})
        reset_pivoted = reset_pivoted.sort_values('count',ascending=False) 
        fig6 = px.bar(reset_pivoted,x='count',color='AU_ranking',y='AU_ranking',title='Breakdown by AU Ranking: '+str(startyear)+' - '+str(endyear)) 
        new_df = df.copy() 
        min_year = new_df['Year'].min()
        max_year = new_df['Year'].max() 
        if startyear < min_year: 
        ...
        fig7 = px.histogram(query,y='Cited by',x='Document Type',title='Citations by Document Type: '+str(startyear)+' - '+str(endyear)) 
        new_df = df.copy() 
        min_year = new_df['Year'].min()
        max_year = new_df['Year'].max() 
        if startyear < min_year: 
        ...
        fig8 = px.pie(reset_pivoted,values='count',names='Publication Stage',title='Breakdown by Publication Stage: '+str(startyear)+' - '+str(endyear)) 
